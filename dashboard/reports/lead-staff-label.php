...
try {
    ...
    $sql = "SELECT 
            ...
            FROM users u
            CROSS JOIN lead_labels ll
            LEFT JOIN lead_label_relations llr ON ll.id = llr.label_id
            LEFT JOIN leads l ON u.id = l.assigned_to AND llr.lead_id = l.id
            WHERE u.role != 'admin'
            " . ($staff_id > 0 ? "AND u.id = ?" : "") . "
            " . ($label_id > 0 ? "AND ll.id = ?" : "") . "
            GROUP BY u.id, u.first_name, u.last_name, ll.id, ll.name, ll.color
            ORDER BY staff_name, label_name";
    ...
    $staff_label_data = $result->fetch_all(MYSQLI_ASSOC);
    ...
    foreach ($staff_label_data as $data) {
        ...
        $staff_data[$data['staff_name']]['labels'][$data['label_name']] = $data['total_leads'];
    }

} catch (Exception $e) {
    error_log("Error fetching staff label data: " . $e->getMessage());
...
<head>
    ...
    <title>Lead Staff Label Report</title>
    <!-- Bootstrap 5 CSS -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-4">
                            <div class="report-card">
                                <h2 class="h5 mb-4">Staff Label Performance</h2>
                                <div class="table-responsive">
                                    <table class="performance-table">
                                        <thead>
                                            <tr>
                                                <th>Staff Member / Label</th>
                                                <th>Total Leads</th>
                                                <th>Period Leads</th>
                                                <th>Converted</th>
