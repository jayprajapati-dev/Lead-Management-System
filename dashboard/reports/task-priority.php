...
// Fetch task priority data
try {
    // Get list of task priorities
    $priorities_sql = "SELECT id, name, color FROM task_priorities ORDER BY display_order";
    ...
    $sql = "SELECT 
            ...
            LEFT JOIN tasks t ON tp.id = t.priority_id
            WHERE 1=1
    ...
    foreach ($priority_data as $priority) {
        $total_all_tasks += $priority['total_tasks'];
        $total_completed += $priority['completed_tasks'];
        $total_period_tasks += $priority['period_tasks'];
        $total_period_completed += $priority['period_completed'];
        $total_overdue += $priority['overdue_tasks'];
        if ($priority['avg_completion_days'] !== null) {
            $total_completion_days += ($priority['avg_completion_days'] * $priority['completed_tasks']);
            $completion_count += $priority['completed_tasks'];
        }
    }
    ...
    error_log("Error fetching task priority data: " . $e->getMessage());
...
<head>
    ...
    <title>Task Priority Report</title>
    <!-- Bootstrap 5 CSS -->
...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-4">
                            <div class="report-card">
                                <h2 class="h5 mb-4">Priority Performance</h2>
                                    ...
                                    <table class="priority-table">
                                        ...
                                        <tbody>
                                            <?php foreach ($priority_data as $priority): ?>
                                                <tr>
                                                    <td>
                                                        <div class="priority-badge" style="background-color: <?php echo htmlspecialchars($priority['priority_color']); ?>20; color: <?php echo htmlspecialchars($priority['priority_color']); ?>;">
                                                            <?php echo htmlspecialchars($priority['priority_name']); ?>
                                                        </div>
                                                    </td>
                                                    <td><?php echo number_format($priority['total_tasks']); ?></td>
                                                    <td><?php echo number_format($priority['period_tasks']); ?></td>
                                                    <td><?php echo number_format($priority['completed_tasks']); ?></td>
                                                    <td>
                                                        <?php
                                                        $rate = $priority['total_tasks'] > 0 ? 
                                                            round(($priority['completed_tasks'] / $priority['total_tasks']) * 100, 1) : 0;
                                                    ...
                                                    <td>
                                                        <?php
                                                        echo $priority['avg_completion_days'] !== null ? 
                                                            round($priority['avg_completion_days'], 1) . ' days' : 'N/A';
                                                        ?>
                                                    </td>
                                                    <td>
                                                        <?php if ($priority['overdue_tasks'] > 0): ?>
                                                            <div class="overdue-count">
                                                                <?php echo number_format($priority['overdue_tasks']); ?>
                                                            </div>
                                                        <?php else: ?>
                                                            -
                                                        <?php endif; ?>
    ...
    <script>
        ...
        const priorityNames = <?php echo json_encode(array_column($priority_data, 'priority_name')); ?>;
        const priorityColors = <?php echo json_encode(array_column($priority_data, 'priority_color')); ?>;
        const totalTasks = <?php echo json_encode(array_column($priority_data, 'total_tasks')); ?>;
        const completedTasks = <?php echo json_encode(array_column($priority_data, 'completed_tasks')); ?>;
        const periodTasks = <?php echo json_encode(array_column($priority_data, 'period_tasks')); ?>;
        const periodCompleted = <?php echo json_encode(array_column($priority_data, 'period_completed')); ?>;
