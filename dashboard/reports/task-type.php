...
try {
    ...
    $types_sql = "SELECT id, name, color FROM task_types ORDER BY name";
    ...
    $sql = "SELECT 
            ...
            FROM task_types tt
            LEFT JOIN tasks t ON tt.id = t.type_id
            WHERE 1=1
            " . ($type_id > 0 ? "AND tt.id = ?" : "") . "
    ...
    if ($type_id > 0) $params[] = $type_id;
    if ($staff_id > 0) $params[] = $staff_id;

    $stmt = executeQuery($sql, $params);
    $result = $stmt->get_result();
    $type_data = $result->fetch_all(MYSQLI_ASSOC);
    ...
    foreach ($type_data as $type) {
        $total_all_tasks += $type['total_tasks'];
        $total_completed += $type['completed_tasks'];
        $total_period_tasks += $type['period_tasks'];
        $total_period_completed += $type['period_completed'];
        $total_overdue += $type['overdue_tasks'];
        if ($type['avg_completion_days'] !== null) {
            $total_completion_days += ($type['avg_completion_days'] * $type['completed_tasks']);
            $completion_count += $type['completed_tasks'];
        }
    }
    ...
    error_log("Error fetching task type data: " . $e->getMessage());
...
<body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 mb-4">
                            <div class="report-card">
                                <h2 class="h5 mb-4">Type Performance</h2>
                                    ...
                                    <table class="type-table">
                                        ...
                                        <tbody>
                                            <?php foreach ($type_data as $type): ?>
                                                <tr>
                                                    <td>
                                                        <div class="type-badge" style="background-color: <?php echo htmlspecialchars($type['type_color']); ?>20; color: <?php echo htmlspecialchars($type['type_color']); ?>;">
                                                            <?php echo htmlspecialchars($type['type_name']); ?>
                                                        </div>
                                                    </td>
                                                    <td><?php echo number_format($type['total_tasks']); ?></td>
                                                    <td><?php echo number_format($type['period_tasks']); ?></td>
                                                    <td><?php echo number_format($type['completed_tasks']); ?></td>
                                                    <td>
                                                        <?php
                                                        $rate = $type['total_tasks'] > 0 ? 
                                                            round(($type['completed_tasks'] / $type['total_tasks']) * 100, 1) : 0;
                                                    ...
                                                    <td>
                                                        <?php
                                                        echo $type['avg_completion_days'] !== null ? 
                                                            round($type['avg_completion_days'], 1) . ' days' : 'N/A';
                                                        ?>
                                                    </td>
                                                    <td>
                                                        <?php if ($type['overdue_tasks'] > 0): ?>
                                                            <div class="overdue-count">
                                                                <?php echo number_format($type['overdue_tasks']); ?>
                                                            </div>
                                                        <?php else: ?>
                                                            -
                                                        <?php endif; ?>
    ...
    <script>
        ...
        const typeNames = <?php echo json_encode(array_column($type_data, 'type_name')); ?>;
        const typeColors = <?php echo json_encode(array_column($type_data, 'type_color')); ?>;
        const totalTasks = <?php echo json_encode(array_column($type_data, 'total_tasks')); ?>;
        const completedTasks = <?php echo json_encode(array_column($type_data, 'completed_tasks')); ?>;
        const periodTasks = <?php echo json_encode(array_column($type_data, 'period_tasks')); ?>;
        const periodCompleted = <?php echo json_encode(array_column($type_data, 'period_completed')); ?>;
        ...
        new Chart(document.getElementById('taskDistributionChart'), {
            ...
            data: {
                ...
                datasets: [{
                    data: totalTasks,
                    backgroundColor: typeColors.map(color => color + '80'),
